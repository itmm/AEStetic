...
window.addEventListener('load', function () {
	...
	function doEncrypt() {
		...
		for (i = 0; i < dependent.length; ++i) { dependent[i] = []; }
			...
			var block = Array(block_size);
			var temp = Array(block_size);
			dependent = Array(block_size);
		...
		var dec = Array(block_size);
		var inv_permute = Array(block_size);
		...
		for (i = 0; i < block_size; ++i) 
		{
			dec[i] = block[i] ^ expanded_key[state.rounds * block_size + i];
			dependent[i] = ['out-' + i, 'expanded-key-' + (state.rounds * block_size + i)];
		}
