var state = null;

window.addEventListener('load', function () {
	'use strict';

	function $(id) { return document.getElementById(id); }

	function readBytes(id, count, name) {
		var $bytes = $(id).getElementsByTagName('span');
		if ($bytes.length != count) {
			alert("not " + count + " bytes in " + name + " but " + $bytes.length);
			return null;
		}

		var result = Array(count);
		for (var i = 0; i < count; ++i) {
			result[i] = parseInt($bytes[i].innerText, 16);
		}

		return result;
	}

	function newTag(tag) { return document.createElement(tag); }
	function newTxt(txt) { return document.createTextNode(txt); }
	function newSpc() { return newTxt(' '); }

	function appendChild($parent, $child, addSpace) {
		if (addSpace) { $parent.appendChild(newSpc()); }
		$parent.appendChild($child);
	}

	function removeChilds($parent) {
		while ($parent.hasChildNodes()) { $parent.removeChild($parent.lastChild); }
	}

	function writeBytes($dest, ary) {
		var len = ary.length;

		removeChilds($dest);

		for (var i = 0; i < len; i += 8) {
			var $div = newTag('div');
			for (var j = 0; j < 8; ++j) {
				var val = ary[i + j];
				var formatted = val < 16 ? '0' + val.toString(16) : val.toString(16);
				var $span = newTag('span');
				$span.appendChild(newTxt(formatted));

				appendChild($div, $span, j);
			}
			appendChild($dest, $div, i);
		}
	}

	state = {
		'sbox': [
		
		],
		'permute': [
		0x00, 0x05, 0x0a, 0x0f,  0x04, 0x09, 0x0e, 0x03,
		0x08, 0x0d, 0x02, 0x07,  0x0c, 0x01, 0x06, 0x0b
		],
		'key': [
		0x00, 0x01, 0x02, 0x03,  0x04, 0x05, 0x06, 0x07,
		0x08, 0x09, 0x0a, 0x0b,  0x0c, 0x0d, 0x0e, 0x0f,
		0x10, 0x11, 0x12, 0x13,  0x14, 0x15, 0x16, 0x17,
		0x18, 0x19, 0x1a, 0x1b,  0x1c, 0x1d, 0x1e, 0x1f
		],
		'input': [
		0x00, 0x11, 0x22, 0x33,  0x44, 0x55, 0x66, 0x77,
		0x88, 0x99, 0xaa, 0xbb,  0xcc, 0xdd, 0xee, 0xff
		]
	};

	function doEncrypt() {
		function aes_mul(a, b) {
			var result = 0;
			while (a != 0)
			{
				if (a & 0x01) { result ^= b; }
				a >>= 1;
				b = (b << 1) ^ (b & 0x80? 0x1b: 0x00);
			}
			return result & 0xff;
		}

		writeBytes($('sbox'), state.sbox);
		writeBytes($('permute'), state.permute);
		writeBytes($('key'), state.key);
		writeBytes($('input'), state.input);

		var expanded_key = Array(240);
		var i, j, k;

		for (i = 0; i < 32; ++i) { expanded_key[i] = state.key[i]; }
			for (i = 32; i < 240; i += 4) {
				for (j = 0; j < 4; ++j) 
				{
					expanded_key[i + j] = expanded_key[i - 4 + j]; 
				}

				if (i % 16 == 0) 
				{
					if (i % 32 == 0)
					{
						k = expanded_key[i];
						for (j = 0; j < 3; ++j) 
						{
							expanded_key[i + j] = expanded_key[i + j + 1]; 
						}
						expanded_key[i + 3] = k;
					}
					for (j = 0; j < 4; ++j) 
					{ 
						expanded_key[i + j] = state.sbox[expanded_key[i + j] & 0xff]; 
					}
					if (i % 32 == 0) { expanded_key[i] ^= 1 << (i/32 - 1); }
				}

				for (j = 0; j < 4; ++j)
				{
					expanded_key[i + j] ^= expanded_key[i + j - 32];
				}
			}

			writeBytes($('expanded-key'), expanded_key);

			var $computation = $('computation');
			removeChilds($computation);

			var block = Array(16);
			var temp = Array(16);
			for (i = 0; i < 16; ++i) block[i] = state.input[i] ^ expanded_key[i];

				function add_tmp(name, block) {
					var $p = newTag('p');
					$p.appendChild(newTxt(name));
					$computation.appendChild($p);
					var $d = newTag('div');
					$d.classList.add('bytes');
					writeBytes($d, block);
					$computation.appendChild($d);
				}

				for (var round = 1; round <= 14; ++round) {
					var $h = document.createElement('h3');
					$h.appendChild(document.createTextNode('round ' + round));
					$computation.appendChild($h);

					for (i = 0; i < 16; ++i) block[i] = state.sbox[block[i]];
						add_tmp('after S-Box:', block);

					for (i = 0; i < 16; ++i) temp[i] = block[state.permute[i]];
						add_tmp('after permutation:', temp);

					if (round < 14) {
						for (j = 0; j < 4; ++j)
						{
							var s0 = temp[4 * j];
							var s1 = temp[4 * j + 1];
							var s2 = temp[4 * j + 2];
							var s3 = temp[4 * j + 3];

							temp[4 * j] = aes_mul(2, s0) ^ aes_mul(3, s1) ^ s2 ^ s3;
							temp[4 * j + 1] = s0 ^ aes_mul(2, s1) ^ aes_mul(3, s2) ^ s3;
							temp[4 * j + 2] = s0 ^ s1 ^ aes_mul(2, s2) ^ aes_mul(3, s3);
							temp[4 * j + 3] = aes_mul(3, s0) ^ s1 ^ s2 ^ aes_mul(2, s3);
						}
						add_tmp('after mult:', temp);
					}

					for (i = 0; i < 16; ++i) 
					{
						block[i] = temp[i] ^ expanded_key[16 * round + i]; 
					}
					add_tmp('after mix with key:', block);
				}
			}

			doEncrypt();
		});